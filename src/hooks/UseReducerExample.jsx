 ...
 * 2- action -> Object -> send to reducer -> dispatch to reducer (type, payload)
...
const UseReducerExample = () => {
   ...
   return (
    <div className="mx-auto mt-5 d-grid gap-3 w-50">
        ...
        <button className="btn btn-primary" onClick={increment}>اضافه کن</button>
        <p className="alert alert-warning text-center">
            شمارش شما برابر است با : {" "}
            <span className="badge rounded-pill bg-success">{state.count}</span>
        </p>
        <button className="btn btn-danger" onClick={decrement}>کم کن</button>
    </div>
   );
}
